...
type DemoBMServer interface {
	...
	Start(ctx context.Context, req *google_protobuf1.Empty) (resp *HelloResp, err error)
}
...
func demoStart(c *bm.Context) {
	p := new(google_protobuf1.Empty)
	if err := c.BindWith(p, binding.Default(c.Request.Method, c.Request.Header.Get("Content-Type"))); err != nil {
		return
	}
	resp, err := DemoSvc.Start(c, p)
	c.JSON(resp, err)
}
