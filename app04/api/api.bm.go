...
var PathLoginServiceLogin = "/kratosL/login"
...
// LoginServiceBMServer is the server API for LoginService service.
type LoginServiceBMServer interface {
	Login(ctx context.Context, req *LoginReq) (resp *LoginResp, err error)
}

var LoginServiceSvc LoginServiceBMServer

func loginServiceLogin(c *bm.Context) {
	p := new(LoginReq)
	if err := c.BindWith(p, binding.Default(c.Request.Method, c.Request.Header.Get("Content-Type"))); err != nil {
		return
	}
	resp, err := LoginServiceSvc.Login(c, p)
	c.JSON(resp, err)
}

// RegisterLoginServiceBMServer Register the blademaster route
func RegisterLoginServiceBMServer(e *bm.Engine, server LoginServiceBMServer) {
	LoginServiceSvc = server
	e.POST("/kratosL/login", loginServiceLogin)
}
